...
rem set file name
set "jar-analyzer=jar-analyzer-2.20.jar"
...
set "jvm_gc=-XX:+UseZGC -Xlog:gc*:file=gc.log:time,uptime:filecount=10,filesize=10M -XX:+UseCompressedOops"
...
rem start jar
%jre_bin_abs% %jvm_gc% %java_agent% %boot_args% %java_args% -cp %java_cp% %main_class% gui -t %theme_name% -p %api_server_port%

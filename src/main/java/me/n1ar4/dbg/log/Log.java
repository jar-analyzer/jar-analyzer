...
class Log {
    ...
    static void log(LogLevel level, String message, String color) {
        if (level.compareTo(LogManager.logLevel) >= 0) {
            ...
            MainForm.getInstance().getLogArea().append(logMessage);
            int len = MainForm.getInstance().getLogArea().getText().length();
            MainForm.getInstance().getLogArea().setCaretPosition(len);
        }
    }
}
...
目前 `SCA` 仅支持 `Apache Log4j2` 全版本的 `CVE` 漏洞匹配（其他后续更新）

更新日志：

- [重要] 支持了 `SCA` 检测
- [SCA] 支持 `Apache Log4j2` 全版本的匹配
- [SCA] 支持 `FAT JAR` 和 `LIB JAR` 两种检测
- [优化] 整体优化 `GUI` 排列布局
...
如果你追求**极致**的性能可以考虑使用 `windows-21` 版本（没有进行完善测试）
...
如果你追求**极致**的性能可以考虑使用 `windows-full-21` 版本（没有进行完善测试）

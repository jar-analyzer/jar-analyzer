...
- [BUG] 修复默认启动的 `GUI` 底部日志区纵向截断 @4ra1n
- [BUG] 修复污点分析失败丢弃已有结果 @4ra1n
- [BUG] 修复 `long/double` 参数污点槽位错位 @4ra1n
- [BUG] 修复部分净化规则配置错误 @4ra1n
- [BUG] 修复构造器场景污点传播丢失 @4ra1n
- [优化] 污点分析支持可变参数传播 @4ra1n
- [优化] 污点分析支持接口默认方法 @4ra1n
- [优化] 异常对象污点传播进入捕获块 @4ra1n
- [优化] 完善污点净化规则覆盖常见安全库 @4ra1n
- [优化] 污点分析结果窗口不再永久置顶 @4ra1n
- [优化] `DFS` 调用链展示优化为可读的代码格式 @4ra1n
- [优化] 补充 `jakarta` 和 `Wrapper` 污点传播规则 @4ra1n
- [其他] 污点分析新增 `17` 个单元测试 @4ra1n
- [其他] 新增规则黄金测试样本项目和对应 `CI` @4ra1n
...
- `windows-full` 版内置开启 `G1GC` 的 `JRE 8` 环境 (主要版本)
- `windows-25` 版内置开启 `ZGC` 的 `JRE 25` 环境（未完全测试）
- `windows-system` 版使用系统 `JRE` 的启动脚本 (推荐安装 `JRE/JDK 8`)
- `zip` 版仅包含简单的 `jar` 文件 (使用 `java -jar` 启动)

不再发布 `Go` 版本的 `MCP` 代码和发布，内置 `Java` 版本 `MCP`

## 6.2 - 已发布最新版

发现 `MacOS + Java 8` 存在字符模糊和乱码问题，研究后确认是 `Java` 自身问题

因此从该版本开始 `MacOS` 强制要求 `Java 11` 以上才能使用

更新日志：

- [功能] 各处黑白名单配置自动保存和加载 @4ra1n
- [功能] 各处黑白名单配置支持右键还原和语法查看 @4ra1n
- [功能] 字体大小支持分别设置面板和代码区域 @4ra1n
- [功能] 字体大小初次设置后支持保存且支持启动后修改 @4ra1n
- [BUG] 跳转内部类时文件树渲染有误 @4ra1n
- [BUG] 重建数据库前关闭旧连接 @4ra1n
- [BUG] 修复删除数据库失败与空库问题 @4ra1n
- [BUG] 修复某些按钮分析期间界面冻结 @4ra1n
- [BUG] 打开资源文件代码区域高亮错误 @4ra1n
- [BUG] 修复反编译 `module-info` 时 `NPE` 问题 @4ra1n
- [BUG] 修复默认启动的 `GUI` 界面存在微小的截断 @4ra1n
- [BUG] `MacOS` 强制要求 `Java 11` 以上 @4ra1n
- [BUG] 修复内置 `FernFlower` 在 `JDK 25` 不兼容 @4ra1n
- [优化] 各处黑白名单配置鼠标光标模糊修复 @4ra1n
- [优化] 分析后无 `class` 高亮提示可能错误配置白名单 @4ra1n
- [优化] `MCP` 面板允许查看工具详情 @4ra1n
- [优化] 优化内部类相关用户交互和展示 @4ra1n
- [优化] 优化 `Markdown` 渲染代码 @4ra1n
- [优化] 优化 `AI` 对话用户体验 @4ra1n
- [其他] 删除体积较大的依赖和不常用功能 @4ra1n
- [其他] 更完善的 `CI` 测试和已有测试优化 @4ra1n
...
- [GHSA-fgw4-24hw-cjm6] 修复 `ZIP SLIP` 漏洞 @4ra1n
- [GHSA-77fr-7cxc-mxqr] 修复 `拒绝服务` 漏洞 @4ra1n
- [GHSA-xxj8-5qwp-rrmh] 修复 `XSS` 漏洞 @4ra1n
- [BUG] 修复 `JList` 渲染不稳定问题 @4ra1n
- [优化] 优化 `AI` 相关功能以及内置 `Prompt` @4ra1n
- [优化] 所有 `GUI Designer` 换成原生 `Swing` @4ra1n
- [优化] 优化 `MCP` 面板 @4ra1n
- [优化] 优化内置小游戏执行框架 @4ra1n
- [其他] 删除某些用不到的测试功能 @4ra1n
- [其他] 更新所有依赖到最新 @4ra1n
- [其他] 升级内置 `JRE` 到 `8u502/25.0.4` @4ra1n
...
重要版本更新，内置 `MCP` 一键启动，内置 `AI` 助手

更新日志：

- [重要] 内置 `MCP` 在面板上进行操作查看 @4ra1n
- [功能] 内置 `AI` 助手，默认集成 `DeepSeek/GLM` @4ra1n
- [功能] 纯 `Java Swing` 实现的 `AI workflow` 分析 @4ra1n
- [功能] 支持 `AI` 分析代码片段，`DIFF` 和 `DFS` 辅助分析 @4ra1n 
- [功能] 信息泄露检查支持 `AI` 研判过滤保留有意义结果 @4ra1n
- [功能] 搜索结果允许导出为 `JSON` 文件 @4ra1n
- [BUG] 数据库连接池耗尽问题处理 @4ra1n
- [BUG] 不同 `JAR` 包含同类名查询时报错问题 @4ra1n
- [优化] 某些 `API` 输入参数进行校验防止预期外错误 @4ra1n
- [优化] 方法跳转代码始终展示在页面中央优化体验 @4ra1n
- [优化] 信息泄露分析加入进度条防止大项目无法观察进度 @4ra1n
- [优化] 优化启动展示图和项目 `LOGO` @4ra1n
- [优化] `CTRL` 左键查 `CALL` 操作优化 @4ra1n
- [优化] `CTRL` 左键操作和一键 `SINK` 操作加入进度条 @4ra1n
- [优化] `DFS` 默认启用空 `SOURCE` 列举 @4ra1n
- [优化] 反编译代码部分线程安全优化 @4ra1n
- [其他] 优化 `CI` 测试 @4ra1n
- [其他] 更新所有依赖到最新 @4ra1n
...
- `windows-full` 版内置开启 `G1GC` 的 `JRE 8` 环境 (主要版本)
- `windows-25` 版内置开启 `ZGC` 的 `JRE 25` 环境（包含核心功能，未完全测试）
- `windows-system` 版使用系统 `JRE` 的启动脚本 (推荐安装 `JRE/JDK 8`)
- `zip` 版仅包含简单的 `jar` 文件 (使用 `java -jar` 启动)
...
- 不再发布 `Go` 版本的 `MCP` 代码和发布，内置 `Java` 版本 `MCP`
- 删除 `n8n workflow` 文档，已经内置了简单的 `AI workflow` 替代

## 5.22

重要更新：重构了表达式大幅提升性能，重构了污点分析，优化了 `DIFF` 功能

更新日志：

- [重要] 表达式搜索代码重构提升效率到秒级 @4ra1n
- [重要] 污点分析引擎重写提高分析能力 @4ra1n
- [功能] 表达式搜素面板重构更美观更方便 @4ra1n
- [功能] 数据库新增索引提高查询效率 @4ra1n
- [功能] 支持完整导出 `DIFF` 记录以便于 `AI` 分析 @4ra1n
- [功能] 支持记忆上次打开窗口的位置和各组件滑动位置 @4ra1n
- [BUG] 修复 `MACOS` 某些快捷键冲突问题 @4ra1n
- [BUG] 修复污点分析模拟 `JVM` 某些细节错误 @4ra1n
- [优化] 文件树不同的文件类型使用不同的图标 @4ra1n
- [优化] 文件树左键操作扩大范围优化操作体验 @4ra1n
- [优化] 代码 `TAB` 显示对同名文件和静态文件的优化 @4ra1n
- [优化] 优化 `DIFF` 引擎大幅提高速度并降低内存消耗 @4ra1n
- [优化] `DIFF` 滚动条不再覆盖高亮显示提升用户体验 @4ra1n
- [优化] 表达式搜索支持了更多的模板参考 @4ra1n
- [优化] 高级配置选项提供完善的描述面板 @4ra1n
- [优化] 查看所有字符串按钮优化界面支持点击跳转 @4ra1n
- [其他] 完善 `CI` 测试流程 @4ra1n
...
支持了 `JAR/JAR 目录` 的 `DIFF` 功能，对比分析源码的变动（从 `plugins` 菜单中打开）

更新日志：

- [重要] 支持对 `JAR/JAR 目录` 的 `DIFF` 功能 @4ra1n
- [BUG] 修复构建数据库时某些情况导致的空指针 @4ra1n
- [优化] 优化日志输出防止某些情况乱码 @4ra1n
- [其他] 代码质量问题修复 @4ra1n
...
- [功能] 重写 `sqlite` 查询面板支持补全等操作 @4ra1n
- [功能] 重写表达式搜索面板支持补全提示等操作 @4ra1n
- [功能] 重写 `CTRL+F` 搜索框支持大小写和正则 @4ra1n
- [功能] `WEB` 分析支持展示各种 `Spring` 配置类 @4ra1n
- [BUG] `UI` 主题切换时没有处理字体大小 @4ra1n
- [BUG] `UI` 主题切换时没有处理代码编辑器主题 @4ra1n
- [BUG] 全局索引搜索某些情况无法双 `SHIFT` 打开 @4ra1n
- [BUG] 全局索引搜索某些情况无法关闭 @4ra1n
- [BUG] 多 `TAB` 编辑器中 `CTRL+F` 搜索无法生效 @4ra1n
- [优化] 所有 `JList/JTable` 使用斑马纹优化用户体验 @4ra1n
- [优化] `GADGET` 分析规则补全 @4ra1n
- [优化] 优化部分 `UI` 提升用户体验 @4ra1n
- [优化] 优化欢迎页面提示内容 @4ra1n
...
- [BUG] 修复低版本 `Java` 环境文件选择打开缓慢问题 @4ra1n
- [功能] 搜索部分支持了选择黑名单或白名单两种过滤方式 @AII12754
- [功能] 表达式支持 `containsInvoke` 和 `excludeInvoke` @AII12754
- [功能] 表达式支持 `nameRegex` 和 `classNameRegex` 正则 @AII12754
- [功能] 表达式支持导出结果到 `CSV` 文件 @AII12754
- [优化] 拖拽功能优化支持拖拽到所有地方都可以加载 @4ra1n
- [优化] 搜索部分黑白名单设置在表达式搜索结果中生效 @AII12754
- [优化] 表达式新增了几个常用的内置模板 @AII12754
- [优化] 首页新增按钮一键跳转快速 `sink` 搜索 @4ra1n
- [其他] 有多标签页后不再需要前后步返回按钮 @4ra1n
- [其他] 删除无意义的 `ObjectInputFilter` 机制 @4ra1n
...
- [重要] 代码区域支持类似 `IDEA` 的操作 `Ctrl+左键` 跳转 @4ra1n
- [重要] 代码区域支持打开多个 `tab` 查看和关闭等功能 @4ra1n
- [优化] 取消启动时检测 `Java 8` 版本弹窗提示 @4ra1n
...
- [BUG] 修复某些损坏的 `class` 文件导致无法继续分析 @Gosiu
- [优化] 文件树允许右键展开所有/折叠所有等操作 @4ra1n
- [优化] 文件树选中本行空白处也应该可以展开菜单 @4ra1n
- [优化] 提示某些情况下文件浏览器卡顿原因 @4ra1n
- [其他] 更新 `tools.jar` 到 `1.8.0_482` @4ra1n
- [其他] 更新内置 `jre` 到 `25.0.2` 版本 @4ra1n
- [其他] 所有依赖升级到支持 `Java 8` 的最新版 @4ra1n
...
- [重要] 重构 `HTTP` 服务端大幅提升 `API/MCP` 效率 @4ra1n
- [BUG] `LINUX/MAC` 某些情况 `API/MCP` 不可用 @4ra1n
- [优化] 使用 `LRU` 缓存提升 `API/MCP` 提取代码效率 @4ra1n
- [其他] 升级内置 `JRE` 到 `8u482` 和 `21.0.10` @4ra1n
- [其他] 升级 `mcp` 和 `report-mcp` 的依赖版本 @4ra1n
- [其他] 所有依赖升级到支持 `Java 8` 的最新版 @4ra1n
...
- [功能] 信息泄露检查允许跳转和高亮配置文件内容 @4ra1n
- [功能] 信息泄露检查允许右键复制值和预览内容 @4ra1n
- [功能] `agent` 支持 `tomcat` 和 `undertow` 两种远程分析 @fantasy
- [BUG] 信息泄露检查配置文件路径显示错误问题 @4ra1n
- [BUG] 修复 `report-mcp` 默认端口错误 (不影响使用) @4ra1n
- [优化] 优化 `tomcat-analyzer` 组件 `UI` 展示 @4ra1n
- [优化] 优化 `API` 错误情况显示更多有效信息 @4ra1n
- [其他] 补充序列化数据分析测试文件和默认数据 @4ra1n
...
- [重要] 提供 `SKILL` 能力进行代码审计 @0cat-r
- [功能] 提供 `DFS` 分析的 `API` 和 `MCP` @4ra1n
- [功能] `DFS` 分析允许导出 @4ra1n
- [功能] `DFS` 分析允许设置最大次数防止分析不停 @4ra1n
- [功能] `DFS` 分析允许设置黑名单类跳过分析 @4ra1n
- [BUG] 修复 `API` 方法反编译定位不准确的问题 @4ra1n
